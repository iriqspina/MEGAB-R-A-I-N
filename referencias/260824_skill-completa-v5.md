# ARQUIVO INTEGRAL DA SKILL v5.7 (pre-corte de 260824)
# Guardado no corte 'tirar peso': a SKILL.md virou roteiro curto e este
# arquivo preserva TODO o texto anterior. Consulte quando o roteiro curto
# nao bastar. Caminhos daqui ja estao no layout v7.0.
...
**v5.7 · 2026-08-22.** Base: v5.6 (260822). Mudou: o relatório ganhou
**3 temas × 2 modos** trocáveis pelo leitor — tema é identidade visual
(`modelos/visuais/temas/NN-*.css`), modo é só claro/escuro, e os eixos são
independentes; cor NÃO se edita mais em `tokens.css`, que virou contrato e
fallback. Regra nova no anti-slop visual: **token de estado nunca vira token de
hierarquia** — trocar de tema é o teste mais barato dessa confusão. E nasceu
`04_visuais/`, acervo de referência **do humano** que script nenhum lê.
Histórico: v5.6 — existe UM relatório por
...
1. Procure, nesta ordem: `MEGABRAIN/` dentro do projeto atual; a variável de
   ...
   em pastas — `memoria/nucleo/` (MEGABRAIN.md, VERSAO.txt, README.md, OFFLINE.md,
   licoes-megabrain.md), `memoria/estado/` (ESTADO.md, HANDOFF.md, DECISOES.md,
   META.md, CHECKLIST, ALINHAMENTO, PROGRESSO.json), `memoria/identidade/`
   (memória pessoal), `00_painel/` (HTML gerados). Cópias de projeto
...
`00_painel/CATALOGO-VISUAL.html`.
...
**Cor sai do tema, não da mecânica.** `modelos/visuais/temas/` tem um arquivo
por tema (identidade: cor, tipo, raio, densidade); o modo claro/escuro é o outro
eixo. A mecânica só consome `var(--paper)`, `var(--ink)`, `var(--ok)`… e por
isso um tema novo não toca em nenhuma peça. Ao criar tema: blocos mutuamente
exclusivos por `:not()` (padrão Pico), nunca `!important`.

⚠️ **Token de estado não é token de hierarquia.** `--signal`/`--ok`/`--warn`
significam *estado*; rótulo, cabeçalho e eyebrow usam `--ink-faint`. Usar a cor
de crítico para decorar passa despercebido num tema só e explode no segundo —
aconteceu na v6.7. Trocar de tema é o teste mais barato dessa confusão.

**Referência visual não mora no código.** `04_visuais/` é do humano
(`00_entrada` → `01_sim/<eixo>` / `02_nao`, mais `03_temas` e `_fontes`).
Nenhum script lê essa pasta — a triagem que o dono fizer lá **é dado**, não
bagunça: nunca reorganize sem pedir. Specimens novos: `bin/mb-specimen.py`.
